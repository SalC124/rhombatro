...
func _ready() -> void:
	...
	heart_label = _make_label("♥ 0", Color(1, 0.2, 0.2))
	club_label = _make_label("♣ 0", Color(0.2, 1, 0.2))
	spade_label = _make_label("♠ 0", Color(0.6, 0.6, 1))
	diamond_label = _make_label("♦ x0", Color(1, 0.8, 0.2))
...
func animate_card(card: Node2D, suit: int, value: int) -> Signal:
	...
	# tick up the right counter mid-animation
